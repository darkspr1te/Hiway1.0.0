...
BOOL DbSetupTotalRead(BOOL bBackupCheck /*= TRUE*/)
{
...
#if 	AE_375X	
	//dbLicense.bOnLineEnroll = TRUE;//定制在线注册使用
	dbLicense.nMaxEnrollCount = gFpMaximum;
#endif
	...
	if (!bRet || (dwCheckSum != *(DWORD*)pby) || SetupTotaldata2Default_flag) //To Default
	{
	        if(gpUserIdHeadData==NULL)
   			 DbUserIdHeadListLoad();
		memset(gpUserIdHeadData, 0, sizeof(st_HeadString)*MaxIdHeadListCount);
		gpUserIdHeadData[0].Index=1;
		strcpy(gpUserIdHeadData[0].Letter,"AB");
		gpUserIdHeadData[1].Index=2;
		strcpy(gpUserIdHeadData[1].Letter,"HX");
		...
		//dbSetupTotal.setSystem.nBaudrate = BAUDRATE_115200;  
		dbSetupTotal.setSystem.ipAddress = UI_IP_MAKE(192, 168, 10, 62);
		...
		dbSetupTotal.setSystem.ipPCBackground = UI_IP_MAKE(192, 168, 10, 120);
		...
		dbSetupTotal.setSystem.bDHCPUse = TRUE;//改动的
		...
		dbSetupTotal.setSystem.nRealSpaceTime = 10;// dbSetupTotal.setSystem.nRealSpaceTime     
...
//	FlashWriteWithBackupPolling(TRUE);
	...
	__flashread_type = FLASHREAD_USERINFO;
        ...
        bRet2 = DbUserIdHeadListLoad(); bRet=bRet && bRet2;printf("DbUserIdHeadListLoad: %d\r\n", bRet2);
